...
const DashboardScreen = ({ currentUser, navigateTo }) => {
    ...
    switch (currentUser?.role) {
        ...
        case ROLES.COMPLIANCE_REVIEWER: return renderComplianceFraudDashboard();
...
const ClaimDetailScreen = ({ navigateTo, params, currentUser, updateClaim }) => {
    ...
    const handleModalSubmit = () => {
        ...
        if (modalType === 'approve') {
            const currentApprovalLevel = WORKFLOW_MILESTONES.find(m => m.id === 'approval')?.status.indexOf(claim.status) || -1;
            const nextApprovalStatus = WORKFLOW_MILESTONES.find(m => m.id === 'approval')?.status[currentApprovalLevel + 1];

            if (nextApprovalStatus) {
                newStatus = nextApprovalStatus;
                newAuditEntry.action = `Approved (Level ${currentApprovalLevel + 1})`;
                newApprovalEntry = {
        ...
        updateClaim(claim.id, {
            ...
            // Update currentApprovers based on newStatus and approval matrix
            currentApprovers: getApproversForStatus(newStatus, claim.payoutAmount)
        });
    ...
    const getApproversForStatus = (status, amount) => {
        if (status.startsWith('Approval Level')) {
            ...
            const approvalLevelIndex = WORKFLOW_MILESTONES.find(m => m.id === 'approval')?.status.indexOf(status) || 0;
    ...
    const canApprove = (currentUser?.role === ROLES.CLAIMS_EXAMINER && claim.status === CLAIM_STATUSES.EXAMINER_REVIEW) ||
                      ...
                      (currentUser?.role === ROLES.FINANCE && claim.status === CLAIM_STATUSES.APPROVAL_LEVEL_3);
    const canReject = [ROLES.CLAIMS_EXAMINER, ROLES.SUPERVISOR, ROLES.SENIOR_ADJUDICATOR].includes(currentUser?.role);
    const canRequestInfo = [ROLES.CLAIMS_EXAMINER, ROLES.INTAKE_SPECIALIST].includes(currentUser?.role);
    ...
    const DocPreviewModal = ({ doc, onClose }) => (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '800px', height: '90%', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ flexGrow: 1, overflow: 'auto', backgroundColor: 'var(--color-sand-50)', borderRadius: 'var(--border-radius-md)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
